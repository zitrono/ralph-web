...
import { convertZodSchemaToJsonSchema, enhanceJsonSchema } from './tools/utils/schema_converter.js';
...
export const connectServer = async (server: McpServer): Promise<void> => {
  ...
  try {
    ...
    if ('on' in transport && typeof (transport as any).on === 'function') {
      (transport as any).on('data', (data: any) => {
      ...
      const originalSend = (transport as any).send;
      if (typeof originalSend === 'function') {
        (transport as any).send = function(data: any) {
    ...
    if ('_handleMessage' in server && typeof (server as any)._handleMessage === 'function') {
      const originalHandleMessage = (server as any)._handleMessage;
      (server as any)._handleMessage = function(message: any) {
...
export const registerTool = (
  ...
  const mcpHandler = async (params: Record<string, unknown>, extra: any) => {
    try {
      ...
      if (typeof params === 'object' && params !== null) {
        ...
        if ((params as any).name) logger.info(`CLAUDE_DESKTOP_DEBUG: params.name: ${(params as any).name}`);
        if ((params as any).parameters) logger.info(`CLAUDE_DESKTOP_DEBUG: params.parameters type: ${typeof (params as any).parameters}`);
        if ((params as any).parameters) logger.info(`CLAUDE_DESKTOP_DEBUG: params.parameters keys: ${Object.keys((params as any).parameters || {}).join(', ')}`);
      ...
      const paramInvestigation = {
        ...
        parametersKeys: typeof (params as any)?.parameters === 'object' ? Object.keys((params as any).parameters) : [],
        argumentsKeys: typeof (params as any)?.arguments === 'object' ? Object.keys((params as any).arguments) : [],
      ...
      logger.rawRequest(
        paramInvestigation,
        `${name}-param-investigation`,
        { timestamp: new Date().toISOString(), source: 'parameter_investigation' }
      );
      ...
      if (name === 'cloze_find_people') {
        ...
        logger.info(`params.freeformquery: ${(params as any).freeformquery}`);
        logger.info(`params.arguments?.freeformquery: ${(params as any).arguments?.freeformquery}`);
        logger.info(`params.parameters?.freeformquery: ${(params as any).parameters?.freeformquery}`);
        ...
        if (typeof params === 'object' && params !== null) {
          for (const key of Object.keys(params)) {
            ...
            if (typeof params[key] === 'object' && params[key] !== null) {
              logger.info(`params[${key}] keys: ${Object.keys(params[key] as object).join(', ')}`);
              
              // Try to find freeformquery in any nested object
              const nestedObj = params[key] as Record<string, any>;
 ...
 */
export const createDebugTool = (server: McpServer): void => {
  ...
  import('./tools/cloze_debug_params.js').then(module => {
    ...
    server.tool(
      'cloze_debug_params',
      'Debug tool for tracing parameter passing',
      {},
    ...
    const debugToolHandler = async (params: Record<string, unknown>, extra: any) => {
      ...
      const paramAnalysis = {
        ...
        nameValue: (params as any)?.name,
        parametersType: typeof (params as any)?.parameters,
        parametersKeys: typeof (params as any)?.parameters === 'object' ? Object.keys((params as any).parameters || {}) : [],
        argumentsType: typeof (params as any)?.arguments,
        argumentsKeys: typeof (params as any)?.arguments === 'object' ? Object.keys((params as any).arguments || {}) : [],
      ...
      logger.rawRequest(
        paramAnalysis,
        'debug-tool-param-analysis',
        { timestamp: new Date().toISOString(), source: 'debug_tool_param_analysis' }
      );
...
export const registerTools = (server: McpServer): void => {
   ...
   */
  const registerToolModule = (server: McpServer, modulePath: string) => {
  ...
  if (!registeredTools.has('cloze_direct_find_people')) {
    import('./tools/cloze_direct_find_people.js').then(module => {
      ...
      const mcpHandler = async (args: Record<string, unknown>, extra: any) => {
        try {
          // Enhanced logging for Claude Desktop debugging
          ...
          if (typeof args === 'object' && args !== null) {
            ...
            // Log special cases for known properties
            if ((args as any).name) logger.info(`CLAUDE_DESKTOP_DEBUG: args.name: ${(args as any).name}`);
            if ((args as any).parameters) logger.info(`CLAUDE_DESKTOP_DEBUG: args.parameters type: ${typeof (args as any).parameters}`);
            if ((args as any).parameters) logger.info(`CLAUDE_DESKTOP_DEBUG: args.parameters keys: ${Object.keys((args as any).parameters || {}).join(', ')}`);
          }
          ...
          if (!args.freeformquery) {
            ...
            const paramValue = 
              (args as any).freeformquery || 
              (args as any).parameters?.freeformquery || 
              (args as any).arguments?.freeformquery ||
              (extra?.request?.params?.arguments as any)?.freeformquery;
            
            if (paramValue) {
              logger.info(`CLAUDE_DESKTOP_DEBUG: Found freeformquery in alternate location: ${paramValue}`);
              (args as any).freeformquery = paramValue;
            } else {
              return {
                content: [
                  {
