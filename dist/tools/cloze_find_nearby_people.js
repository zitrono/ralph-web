/**
 * Cloze Find Nearby People Tool
 * Finds people near a specific location
 */
import { z } from 'zod';
import { findNearbyPeople } from '../api/endpoints/people.js';
import { createToolHandler } from './utils/index.js';
import logger from '../logging.js';
/**
 * Parameter schema for the cloze_find_nearby_people tool
 */
...
const handler = async (params) => {
    logger.info('Finding people near location:', params.location);
    // Extract location and pagination parameters
    const { location, ...paginationParams } = params;
    // Call the API to find nearby people
    const response = await findNearbyPeople(location, paginationParams);
    logger.info(`Found ${response.people.length} people near "${location}"`);
    return response;
};
...
export const metadata = {
    name: 'cloze_find_nearby_people',
    description: `Finds people near a specific location.
...
//# sourceMappingURL=cloze_find_nearby_people.js.map