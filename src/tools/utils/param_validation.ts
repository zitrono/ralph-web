 ...
 */
export const withParamValidation = <T>(
  schema: z.ZodType<T>,
...
export const personSegmentSchema = z.enum(PEOPLE_SEGMENTS as [string, ...string[]]).optional()
  .describe(`Person segment (${PEOPLE_SEGMENTS.join(', ')})`);

// Create schema for validating person stage
export const personStageSchema = z.enum(PEOPLE_STAGES as [string, ...string[]]).optional()
  .describe(`Person stage (${PEOPLE_STAGES.join(', ')})`);

// Create schema for validating project segment
export const projectSegmentSchema = z.enum(PROJECT_SEGMENTS as [string, ...string[]]).optional()
  .describe(`Project segment (${PROJECT_SEGMENTS.join(', ')})`);

// Create schema for validating project stage
export const projectStageSchema = z.enum(PROJECT_STAGES as [string, ...string[]]).optional()
...
export const isValidEnumValue = (
  ...
  if (!value) return true; // undefined is valid for optional fields
