...
export async function testPeopleWorkflow(server) {
    ...
    try {
        ...
        if (!person.name.includes('TEST') ||
            !person.emails.some((e) => e.value.includes(email.split('@')[0]))) {
        ...
        // Process response
        let processedUpdateResult = updateResult;
        if (updateResult.content && Array.isArray(updateResult.content)) {
            try {
                const textContent = updateResult.content.find((item) => item.type === 'text' && item.text);
                if (textContent && textContent.text) {
        ...
        let processedFindAfterUpdate = findAfterUpdateResult;
        if (findAfterUpdateResult.content && Array.isArray(findAfterUpdateResult.content)) {
            try {
                const textContent = findAfterUpdateResult.content.find((item) => item.type === 'text' && item.text);
                if (textContent && textContent.text) {
        ...
        // Process the response
        let processedDeleteResult = deleteResult;
        if (deleteResult.content && Array.isArray(deleteResult.content)) {
            try {
                const textContent = deleteResult.content.find((item) => item.type === 'text' && item.text);
                if (textContent && textContent.text) {
        ...
        // Process the response
        let processedFindAfterDelete = findAfterDeleteResult;
        if (findAfterDeleteResult.content && Array.isArray(findAfterDeleteResult.content)) {
            try {
                const textContent = findAfterDeleteResult.content.find((item) => item.type === 'text' && item.text);
                if (textContent && textContent.text) {
        ...
        if (processedFindAfterDelete.people && processedFindAfterDelete.people.length > 0) {
            ...
            const matchingPerson = processedFindAfterDelete.people.find((p) => p.emails && p.emails.some((e) => e.value.includes(email.split('@')[0])) &&
    ...
    // Process the response
    let processedFinalVerification = finalVerificationResult;
    if (finalVerificationResult.content && Array.isArray(finalVerificationResult.content)) {
        try {
            const textContent = finalVerificationResult.content.find((item) => item.type === 'text' && item.text);
            if (textContent && textContent.text) {
    ...
    if (processedFinalVerification.people && processedFinalVerification.people.length > 0) {
        // Check if any of the found people match our test person
        const matchingPerson = processedFinalVerification.people.find((p) => p.emails && p.emails.some((e) => e.value.includes(email.split('@')[0])) &&
            p.name.includes('TEST'));
        if (matchingPerson) {
...
//# sourceMappingURL=people_workflow.js.map