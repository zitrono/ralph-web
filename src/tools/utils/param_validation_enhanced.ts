 ...
 */
export const preprocessParams = (params: any): any => {
  ...
  if (typeof params === 'string') {
    try {
      ...
      if (parsed.params?.arguments) return parsed.params.arguments;
...
export const getParameterExamples = (
  paramName: string,
  schemaEnhancements: Record<string, { examples?: any[] }>
): string[] => {
  if (!schemaEnhancements || !schemaEnhancements[paramName] || !schemaEnhancements[paramName].examples) {
    return [];
  }
  
  return schemaEnhancements[paramName].examples.map(example => 
 ...
 */
export const createMissingParameterError = (
  paramName: string,
  schemaEnhancements?: Record<string, { examples?: any[], description?: string }>
): ValidationErrorResponse => {
      ...
      if (!params || Object.keys(params).length === 0) {
        ...
        try {
          ...
          if (schema.description) {
            const description = schema.description as any;
 ...
 * Handles various MCP formats including Claude's format
 */
export const createToolHandlerWithEnhancedValidation = <P>(
  schema: z.ZodType<P>,
  schemaEnhancements: Record<string, { examples?: any[], description?: string }>,
  ...
  const handlerWithFormatting = async (params: P): Promise<any> => {
    try {
