 ...
 */
function createTestParams(toolName: string, paramFormat: string, baseParams: Record<string, any>): Record<string, any> {
  switch (paramFormat) {
    case 'direct':
 ...
 */
function saveResultsToLog(logFile: string, data: any): void {
...
async function main() {
  ...
  try {
    ...
    (testParams as any)._raw_test = true;
